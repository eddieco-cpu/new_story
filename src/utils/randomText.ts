...
export default function generateRandomText(
  ...
  options = options || {};
  const usePunc = options.usePunc === undefined ? true : options.usePunc;
  const str: string[] = [];
  const length = Math.floor(Math.random() * (max - min + 1)) + min || 10;
  let i = 0;
  let puncCounter = 0;
  let lengthToNextPunc =
    Math.floor(Math.random() * (maxlengthToNextPunc - minlengthToNextPunc)) +
    minlengthToNextPunc;

  while (i < length) {
    const chosen = Math.floor(Math.random() * vocabSize);
